...
public class McpService {
	 ...
	 * @return 成功时返回 OSS 访问说明或对象 URL，失败时返回错误信息
	 */
	@Tool(description = "从给定的 MP4 下载地址下载文件并上传到指定的 OSS。入参：mp4Url 必填；ossBucket、ossObjectKey 可选，不填则使用配置的 bucket 和从 URL 解析的文件名。")
	public String downloadMp4AndUploadToOss(String mp4Url, String ossBucket, String ossObjectKey) {
		if (mp4Url == null || mp4Url.isBlank()) {
			return "错误：mp4Url 不能为空";
		}
		...
		if (bucket.isBlank() || ossEndpoint.isBlank() || ossAccessKeyId.isBlank() || ossAccessKeySecret.isBlank()) {
			return "错误：OSS 未配置完整，请在 application.properties 或环境变量中配置 oss.endpoint、oss.access-key-id、oss.access-key-secret、oss.bucket-name";
		}
		OSS ossClient = null;
		try {
			ResponseEntity<Resource> response = restTemplate.getForEntity(URI.create(mp4Url.trim()), Resource.class);
			if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
				return "下载失败：HTTP " + response.getStatusCode() + "，或响应体为空";
			}
			...
			String region = ossEndpoint.contains("//") ? ossEndpoint.substring(ossEndpoint.indexOf("//") + 2).split("\\.")[0] : "oss";
			return "上传成功。Bucket=" + bucket + ", ObjectKey=" + objectKey
					+ ", 可访问路径: https://" + bucket + "." + region + ".aliyuncs.com/" + objectKey;
		} catch (Exception e) {
